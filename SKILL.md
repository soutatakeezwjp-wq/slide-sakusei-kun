...
操作説明、画面説明、クリック手順、設定手順、アップロード、共有、ログイン、Studio、ツール選択など、実際のUIが重要なスライドでは、Image genで架空UIを作らない。

- 概念説明やイメージ説明のスライドはImage genで作る。
- 実操作が必要なスライドだけ、実画面スクリーンショット方式を使う。
- 可能な場合は実サイトにアクセスし、デスクトップ幅の画面でスクリーンショットを取得する。
- スクリーンショットはスマホ幅や細長いサイドバー表示ではなく、原則として横長デスクトップ表示で撮影する。
- 実サイトにアクセスできない、ログインが必要、または個人情報が映る場合は、ユーザー提供のスクリーンショットやデモ用画面を使う。
- スクリーンショット内に個人情報、顧客情報、メールアドレス、非公開データが映る場合は、必ず隠すか、デモ用データで撮影する。
- スクリーンショットは中央に大きく配置し、矢印、番号、囲み枠、短い説明文を後処理またはPPTX図形で追加する。
- 矢印、番号、囲み枠、注釈文はImage genに描かせず、機械的に重ねる。
- 上下バーは通常スライドと同じく `#78a9ff` で機械的に追加する。

判断例:

- 「NotebookLMとは」「活用シーン」「できること」: Image gen向き。
- 「ソースを追加する」「マインドマップを押す」「共有設定を開く」「Studioでスライドを作る」: 実スクショ向き。
